...
export const scaleForDuration = (dateRange: DateRangePart): DisplayScale => {
	const diff = dateRange.toDateTime
		.diff(dateRange.fromDateTime)
		.as('seconds');
	if (diff < 60) {
		return 'second';
	}
...
export function dateTimeToString(
	...
	if (isMonthStartOrEnd(dateTime, scale)) {
		if (isStartDate) {
			...
			const adjustedMonth =
				adjustedBack.month < 10
					? '0' + adjustedBack.month
					: adjustedBack.month;
			return dateFormat
